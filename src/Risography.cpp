...
Risography::Risography(std::string desc, std::string paper, double format,
                       int copy, int layersA, int layersB, int staples, int cut,
                       int fold, double labor_nb, double graphism_nb,
                       double imposition_nb, double discount, bool fee,
                       double price_ink, double price_paper,
                       double price_master, double price_graphism,
                       double price_shaping, double price_imposition,
                       double price_labor, int sheet_weight)
    : description(desc), paper(paper), format(format), copy(copy),
      layersA(layersA), layersB(layersB), staples_per_unit(staples),
      cut_per_unit(cut), fold_per_unit(fold), labor_amount(labor_nb),
      graphic_amount(graphism_nb), imposition_amount(imposition_nb),
      discount_percentage(discount),
      shipping_fees(fee ? std::numeric_limits<double>::infinity() : 0.0),
      ink_price(price_ink), sheet_price(price_paper),
      master_price(price_master), graphic_price(price_graphism),
      shaping_price(price_shaping), imposition_price(price_imposition),
      labor_price(price_labor), sheet_weight(sheet_weight), sheet_quantity(0),
      staple_quantity(0), masters_quantity(0), run_quantity(0), weight(0.0),
      total(0.0), unit_cost(0), facturier(nullptr) {
  facturier = new Facturier;
  if (!facturier)
    throw std::runtime_error("facturier not ready");

  facturier->verbose("New print job created");
  std::map<std::string, double> consumableList =
      facturier->getConsumablePrices();
  std::map<std::string, std::pair<int, double>> paperList =
      facturier->getPaperPrices();
...
Risography::Risography(Facturier *f)
    : description(""), paper(""), format(0), copy(0), layersA(0), layersB(0),
      staples_per_unit(0), cut_per_unit(0), fold_per_unit(0), labor_amount(0),
      graphic_amount(0), imposition_amount(0), discount_percentage(0),
      shipping_fees(0), ink_price(0), sheet_price(0), master_price(0),
      graphic_price(0), shaping_price(0), imposition_price(0), labor_price(0),
      sheet_weight(0), sheet_quantity(0), staple_quantity(0),
      masters_quantity(0), run_quantity(0), weight(0), total(0), unit_cost(0),
      facturier(f) {
  if (!f)
    throw std::runtime_error("facturier not ready");
...
double Risography::total_shipping() {
  ...
  if (!paper.empty() && sheet_weight == 0) {
    sheet_weight =
        static_cast<double>(facturier->getPaperPrices().at(paper).first);
  } else if (sheet_weight == 0) {
    sheet_weight = static_cast<double>(
        facturier->getPaperPrices().at("evercopy.90").first);
  }
...
double Risography::total_shaping() {
  ...
  return (staples_per_unit + fold_per_unit + cut_per_unit) * copy *
         shaping_price;
}
...
std::string Risography::generate_stream() {
  ...
  ss << "    Intitulé: " << description
     << end_str(22, offset - description.length());
  ...
  std::string feesStr =
      (total_shipping() > 0.0) ? fit(total_shipping(), 8) : fit("--", 8);
  ...
  ss << new_line("encre (passages)", sheet_quantity * masters_quantity,
                 total_ink());
  ss << new_line("plis/coupe/agrafe", staple_quantity + fold_per_unit,
                 total_shaping());
  ss << "    |---------------------------------------|" << end_str();
  ss << new_line("impression", labor_amount, total_labor());
  ss << new_line("graphisme", graphic_amount, total_graphic());
  ss << new_line("imposition", imposition_amount, total_imposition(),
                 end_str());
  ss << "    |---------------------------------------|" + end_str();
...
// Risography &Risography::operator=(const Risography &other) {(void) other;
// return (*this);}
