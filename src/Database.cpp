...
Database::Database(const std::string& path, Facturier* f): _db(nullptr), _facturier(f) {
...
sqlite3_int64 Database::insert_document(Facture* facture, sqlite3_int64 client_id) {
    ...
    if (numero != facture->number) {
        std::cout << "numero conflict, using " << numero << " instead of " << facture->number << '\n';
    ...
    switch (facture->type) {
        case Facture::TYPE::FACTURE: type = "facture"; break;
        case Facture::TYPE::DEVIS:   type = "devis";   break;
        case Facture::TYPE::TICKET:  type = "ticket";  break;
    }

	switch(facture->status) {
		case Facture::STATUS::DRAFT: status = "draft";
		case Facture::STATUS::SENT: status = "sent";
		case Facture::STATUS::APPROVED: status = "approved";
		case Facture::STATUS::CONVERTED: status = "converted";
	}
...
std::string Database::make_unique_numero(const std::string& base_numero) {
    ...
    for (int i = 0; i < 4096; ++i) {
        ...
        oss << prefix
            << std::uppercase << std::hex << std::setw(3) << std::setfill('0') << counter
		...
		do {
			...
			switch(choice) {
				...
				case 3: 
					...
					update_document(selection, Facture::TYPE::FACTURE, Facture::STATUS::CONVERTED);
					break;
				case 4: {
					std::string folder = f->type == Facture::TYPE::FACTURE ? "factures/" : "devis/";
					folder = "documents/" + folder;
					make_png(f->generate_stream(), _facturier->get_font(), folder + f->get_filename());
					break;
						}
				case 5: {
					std::string folder = f->type == Facture::TYPE::FACTURE ? "factures/" : "devis/";
					...
					send_email_smtp(
						...
						_facturier->get_config("from"),
						_facturier->get_config("url"),
						_facturier->get_config("user"),
...
Client* Database::get_client(sqlite3_int64 client_id) {
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		if (name.empty()) throw std::logic_error("No name for this client\n");
...
std::vector<Risography*> Database::get_risography_of(sqlite3_int64 document_id) {
    const char* sql = R"(
        SELECT
            id,
            description,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        Risography* r = new Risography(desc, paper, format, copy, layersA, layersB,
...
void Database::print_risography(sqlite3_int64 document_id, PrintMode mode) {
	...
	for (size_t i = 0; i < prints.size(); i++) {
 		if (mode == COMPLETE) {
			...
			std::cout << "    Riso print : " << prints[i]->description << " (" << prints[i]->rest_to_pay() << "€)\n";
		}
		delete prints[i];
	}
}
...
void Database::print_all_documents(PrintMode mode, std::optional<Facture::TYPE> type_filter) {
    sqlite3_stmt* stmt = nullptr;
    ...
    if (!type_filter.has_value()) {
        ...
        switch (type_filter.value()) {
            case Facture::TYPE::FACTURE: type_str = "facture"; break;
            case Facture::TYPE::DEVIS:   type_str = "devis";   break;
            case Facture::TYPE::TICKET:  type_str = "ticket";  break;
        }
...
bool Database::update_document(sqlite3_int64 document_id,
    ...
    if (sqlite3_step(get_stmt) == SQLITE_ROW) {
        ...
        std::cerr << "update_document: document id=" << document_id << " not found\n";
    ...
    switch (type) {
        case Facture::TYPE::DEVIS:   suffix = 'D'; type_str = "devis";   break;
        case Facture::TYPE::FACTURE: suffix = 'F'; type_str = "facture"; break;
        case Facture::TYPE::TICKET:  suffix = 'T'; type_str = "ticket";  break;
    }
	switch (status) {
		case Facture::STATUS::DRAFT:  status_str = "draft";
		case Facture::STATUS::SENT:  status_str = "sent";
		case Facture::STATUS::CONVERTED:  status_str = "converted";
		case Facture::STATUS::APPROVED:  status_str = "approved";
	}
